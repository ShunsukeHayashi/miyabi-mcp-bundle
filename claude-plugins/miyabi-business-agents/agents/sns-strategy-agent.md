...
flowchart TB
    subgraph Inputs["📥 入力"]
        persona[ペルソナシート<br/>Phase 3]
        funnel[導線設計<br/>Phase 7]
        content[コンテンツ計画<br/>Phase 6]
        template[テンプレート]
    end

    subgraph SNSAgent["📱 SNSStrategyAgent"]
        direction TB

        subgraph Analysis["🔍 分析フェーズ"]
            platform_analysis[プラットフォーム分析]
            audience_analysis[オーディエンス分析]
            competitor_analysis[競合分析]
        end

        subgraph Strategy["📋 戦略立案フェーズ"]
            platform_select[プラットフォーム選定]
            content_pillars[コンテンツピラー設計]
            tone_manner[トーン&マナー定義]
            hashtag[ハッシュタグ戦略]
        end

        subgraph Planning["📅 計画フェーズ"]
            calendar[投稿カレンダー作成]
            engagement[エンゲージメント計画]
            influencer[インフルエンサー選定]
        end
    ...
    subgraph Outputs["📤 出力"]
        strategy_doc[sns-strategy.md]
        calendar_doc[content-calendar.md]
        engagement_doc[engagement-plan.md]
        influencer_doc[influencer-list.md]
    end

    persona --> platform_analysis
    funnel --> audience_analysis
    content --> competitor_analysis
    template --> Strategy

    Planning --> strategy_doc
    Planning --> calendar_doc
    Planning --> engagement_doc
    Planning --> influencer_doc

    style SNSAgent fill:#e8f5e9,stroke:#4caf50,stroke-width:2px
...
```mermaid
stateDiagram-v2
    [*] --> Initializing: Agent起動

    Initializing --> LoadingInputs: 初期化完了
    LoadingInputs --> AnalyzingPlatforms: 入力読み込み完了

    state AnalysisPhase {
        AnalyzingPlatforms --> AnalyzingAudience: プラットフォーム分析完了
        AnalyzingAudience --> AnalyzingCompetitors: オーディエンス分析完了
        AnalyzingCompetitors --> PlatformSelection: 競合分析完了
    }

    state StrategyPhase {
        PlatformSelection --> DefiningPillars: プラットフォーム選定完了
        DefiningPillars --> SettingTone: ピラー定義完了
        SettingTone --> HashtagStrategy: トーン設定完了
        HashtagStrategy --> CalendarCreation: ハッシュタグ戦略完了
    }

    state PlanningPhase {
        CalendarCreation --> EngagementPlan: カレンダー作成完了
        EngagementPlan --> InfluencerSelection: エンゲージメント計画完了
        InfluencerSelection --> DocumentGeneration: インフルエンサー選定完了
    }

    DocumentGeneration --> Completed: ドキュメント生成完了

    AnalyzingPlatforms --> Escalation: ペルソナSNS不明
    PlatformSelection --> Escalation: 選定困難

    Escalation --> [*]: CoordinatorAgentへ
    Completed --> [*]: 成功終了
```
...
```mermaid
flowchart TD
    start([開始]) --> persona_check{ペルソナの<br/>年齢層は？}

    persona_check -->|10-20代| young[若年層向け]
    persona_check -->|20-40代| middle[ミドル層向け]
    persona_check -->|40代以上| senior[シニア層向け]

    young --> young_platforms["🎵 TikTok優先<br/>📸 Instagram<br/>🐦 X/Twitter"]
    middle --> middle_platforms["📸 Instagram優先<br/>🐦 X/Twitter<br/>📺 YouTube"]
    senior --> senior_platforms["📘 Facebook優先<br/>📺 YouTube<br/>🐦 X/Twitter"]

    young_platforms --> business_check{ビジネス<br/>タイプは？}
    middle_platforms --> business_check
    senior_platforms --> business_check

    business_check -->|B2C| b2c["視覚重視<br/>Instagram/TikTok強化"]
    business_check -->|B2B| b2b["専門性重視<br/>LinkedIn/X強化"]
    business_check -->|Both| both["バランス配分<br/>全プラットフォーム活用"]

    b2c --> resource_check{週あたり<br/>投稿可能数}
    b2b --> resource_check
    both --> resource_check

    resource_check -->|3回未満| minimal["メインPF 1つ<br/>集中戦略"]
    resource_check -->|3-7回| moderate["メインPF 2つ<br/>標準戦略"]
    resource_check -->|7回以上| full["メインPF 3つ<br/>フル展開"]
...
```mermaid
sequenceDiagram
    participant SNS as SNSStrategyAgent
